...
x, y = entrance
path = [entrance]
valid_positions = set()  # Store all valid positions
...
# Remove walls along the path and add valid positions
for (x, y) in path:
...
for _ in range(500):
    ...
    valid_positions.add((n, m))  # Mark position as valid
    canvas.create_line(n * cell_size + 1, m * cell_size, 
                       (n + 1) * cell_size, m * cell_size, fill='white')
for _ in range(500):
    n = random.randint(0, grid_size - 1)
    m = random.randint(0, grid_size - 1)
    valid_positions.add((n, m))  # Mark position as valid
    canvas.create_line(n * cell_size, m * cell_size + 1, 
                       n * cell_size, (m + 1) * cell_size, fill='white')
...
def move_character(event):
    ...
    if event.keysym == "Up" and (x, y - 1) in valid_positions:
        new_position = (x, y - 1)
    elif event.keysym == "Down" and (x, y + 1) in valid_positions:
        new_position = (x, y + 1)
    elif event.keysym == "Left" and (x - 1, y) in valid_positions:
        new_position = (x - 1, y)
    elif event.keysym == "Right" and (x + 1, y) in valid_positions:
        new_position = (x + 1, y)
    else:
